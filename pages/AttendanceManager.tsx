...
const AttendanceManager: React.FC = () => {
  ...
  // Search by Phone ONLY as per rules
  const filteredPresent = presentList.filter(s => s.phone.includes(presentSearch));
  const filteredAbsent = absentList.filter(s => s.phone.includes(absentSearch));
            ...
            <table className="w-full text-right min-w-[400px]">
              <thead>
                <tr className="bg-brand/5 text-brand font-black text-[9px] md:text-[10px] uppercase tracking-widest border-b border-brand/10">
                  <th className="p-4 md:p-5 text-right">اسم الطالب</th>
